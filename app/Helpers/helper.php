...
class helper
{
    ...
    public static function verificationemail($email, $otp)
    {
        ...
        */
        ...
        try {
            ...
            $mail->setFrom('noreply@sysqbit.com', 'SYSQBIT');
            $mail->addAddress($email);
            $mail->addReplyTo('noreply@sysqbit.com', 'SYSQBIT');
    ...
    public static function resendotp($email, $otp)
    {
        ...
        try {
            ...
            $mail->Host = env('MAIL_HOST');             //  smtp host
            $mail->SMTPAuth = true;
            // $mail->Username = 'admin@medicalquizz.com';   //  sender username
            // $mail->Password = 'i;57wk]@e7f?';       // sender password
            $mail->Username = env('MAIL_USERNAME');   //  sender username
            $mail->Password = env('MAIL_PASSWORD');       // sender password
            $mail->SMTPSecure = 'tls';                  // encryption - ssl/tls
            $mail->Port = 587;                          // port - 587/465
            $mail->setFrom('admin@medicalquizz.com', 'SenderName');
            $mail->addAddress($email);
            $mail->addReplyTo('admin@medicalquizz.com', 'SenderReplyName');
